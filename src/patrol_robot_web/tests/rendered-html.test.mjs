...
test("includes live ROS controls and both map implementations", async () => {
  ...
  assert.match(nav2, /plugin_lib_names:/);
  assert.match(nav2, /nav2_compute_path_to_pose_action_bt_node/);
  assert.match(nav2, /progress_checker_plugin: progress_checker/);
  ...
  assert.match(page, /startManual\(0, telemetry\.speed_control_max_angular, "左转"\)/);
  assert.match(page, /startManual\(0, -telemetry\.speed_control_max_angular, "右转"\)/);
  assert.match(page, /speed_control_min_linear/);
  assert.match(page, /speed_control_max_linear/);
  assert.match(page, /网页速度同时约束人工驾驶和自动巡检/);
  assert.match(nav2, /nav2_navfn_planner\/NavfnPlanner/);
  assert.match(nav2, /use_astar: false/);
  assert.match(nav2, /nav2_controller::SimpleProgressChecker/);
