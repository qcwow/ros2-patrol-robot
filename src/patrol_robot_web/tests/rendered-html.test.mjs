...
test("includes live ROS controls and both map implementations", async () => {
  ...
  assert.match(patrolTree, /<SmoothPath/);
  assert.match(patrolTree, /check_for_collisions="true"/);
  ...
  assert.match(nav2, /plugins: \[static_layer, inflation_layer\]/);
  ...
  assert.match(patrolTree, /number_of_retries="1"/);
  assert.match(patrolTree, /BackUp backup_dist="0\.25"/);
